...
def get_dataset_without_custom_outliers(dataset_df: pd.DataFrame, df_column_names: List[str]) -> pd.DataFrame:
    '''Returns a copy of a dataframe with removed outliers.

    Args:
        dataset_df (pd.DataFrame): Input dataframe containing the dataset.
        df_column_names (List[str]): List of column names in the dataframe.
    ...
    print(f'Changed {n_outliers} outlier votes of 1 to 0')
    ...
    for filter in filters:
        ...
        print(f"{filter[0]}: {len(indexes)} observations have been removed")
    # Drop columns 'unknown' and 'no-face'
    dataset_df = dataset_df.iloc[:, :-2]

    return dataset_df

def get_dataset_without_original_outliers(dataset_df: pd.DataFrame,
                                         cross_entropy: bool,
                                         df_column_names: List[str]) -> pd.DataFrame:
    '''Removes outliers from dataframe as in the original Fer-Plus repository at
    https://github.com/microsoft/FERPlus.

    Args:
        dataset_df (pd.DataFrame): Input dataframe containing the dataset.
        cross_entropy (bool): Whether to apply cross-entropy or majority preprocessing.
        df_column_names (List[str]): List of column names in the dataframe.

    Returns:
        pd.DataFrame: A dataframe without outlier records and votes.
    '''
    preprocessed_list = []

    for _, row in dataset_df.iterrows():
        original_votes = list(row[3:])
        processed_votes = _process_votes(original_votes, cross_entropy)
...
def _process_votes(original_votes: List[float], cross_entropy: bool) -> List[float]:
    '''Applies original outlier preprocessing of votes to the single record.

    Args:
        original_votes (List[float]): List of integer vote counts for 10 labels.
        cross_entropy (bool): Whether to apply cross-entropy or majority preprocessing.

    Returns:
        List[float]: Processed votes list for a single record.
    '''
