...
if st.button('Stop Camera'):
    ...
    fig, ax = plt.subplots()
    ax.bar(st.session_state.emotion_scores.keys(), st.session_state.emotion_scores.values(), color='skyblue')
    ax.set_xlabel('Emotion')
    ax.set_ylabel('Total Score')
    ax.set_title('Total Emotion Score')
    st.pyplot(fig)
    
    # Plot Line Graph of Emotion over Time
    st.subheader("Emotion Over Time")
    emotion_df = pd.DataFrame(st.session_state.emotion_data, columns=emotions)
    fig, ax = plt.subplots()
    for emotion in emotions:
        ax.plot(emotion_df.index, emotion_df[emotion], label=emotion)
