...
def display_majority_predictions(images, labels, class_mapping, label_ps=None):
    '''Plots majority predictions for 24 images.
    If it does not receive predicted labels, it simply prints images with true labels.

    Args:
        images(tensor or ndarray): a batch of (batch_size, 1, 48, 48) or (batch_size, 48, 48) images
    ...
    # Convert images to numpy if they are tensors
    if hasattr(images, 'numpy'):
        images = images.numpy()

    # Squeeze the channel dimension if necessary
    if images.ndim == 4:
        images = images.squeeze(1)  # Shape: (batch_size, 48, 48)
...
def display_cross_entropy_predictions(images, labels, class_mapping, label_ps=None):
